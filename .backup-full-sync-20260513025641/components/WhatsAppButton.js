...
function WhatsAppButton() {
    const [telefono, setTelefono] = React.useState('5355002272');
    const [nombreNegocio, setNombreNegocio] = React.useState('');

    React.useEffect(() => {
        window.getTelefonoDuenno().then(tel => {
            setTelefono(window.normalizarTelefonoWhatsApp ? window.normalizarTelefonoWhatsApp(tel) : tel);
        });
    ...
    return (
        <a 
            href={telefono ? `https://api.whatsapp.com/send?phone=${telefono}&text=Hola%2C%20quiero%20consultar%20sobre%20turnos` : '#'} 
            target="_blank" 
